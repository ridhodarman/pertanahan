...
if(!in_array($ext,$ekstensi) ) {
	header("location:../../format-edit.php?format=".$_POST['id']."&gagal=Gagal input data, File risalah harus dalam format rtf");
}
else {
	...
	move_uploaded_file($_FILES['file_ris']['tmp_name'], '../../format/'.$file_ris);

	$cek_upload="../../format/".$file_ris;
	if (file_exists($cek_upload)) {
		$query = "UPDATE format_ptsl SET risalah=? WHERE id=?";
		$sql = $koneksi->prepare($query);
		$sql->bind_param("ss", $file_ris, $id);

		if ($sql->execute()) {
			header("location:../../format-edit.php?format=".$_POST['id']."&alert=File risalah berhasil di-upload");
		}
	}
}