	...
	<div class="container">
		<div class="row">
			<div class="col-md-12">
                ...
                $sql1 = "SELECT id, nomor_berkas, nama_pemohon, desa_nagari, kecamatan, tahun FROM berkas_ptsl ";
                $sql2 = "";
                $sql3 =" ORDER BY waktu_entri DESC limit ?, ?";

                if ($jenis=="desa") {
                	$sql2 = "WHERE LOWER(desa_nagari) LIKE ?";
                }
                else if ($jenis=="nomor") {
                	$sql2 = "WHERE nomor_berkas LIKE ?";
                }
                else if ($jenis=="nama") {
                	$sql2 = "WHERE LOWER(nama_pemohon) LIKE ?";
                }

                $query = $sql1.$sql2.$sql3;
                $sql = $koneksi->prepare($query);
                $sql->bind_param("sss", $cari2, $halaman_awal, $batas);
                $sql->execute();
                $data = $sql->get_result();
                $no = $halaman_awal + 1;
                  ...
                  <a class="page-link" <?php if ($halaman < $total_halaman) {
									...
									<div class="form-row">
										<div class="form-group col-xl-2 col-lg-3 col-md-6 align-self-center">
											<a href="index.php">
												<button type="submit" class="btn btn-secondary active w-100">Kembali Ke Halaman Sebelumnya</button>
											</a>
										</div>
									</div>
							</div>
